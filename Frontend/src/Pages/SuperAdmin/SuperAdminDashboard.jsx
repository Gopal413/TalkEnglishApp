...
import { getSuperAdminStats } from '../../api/superAdminApi';
...
export default function SuperAdminDashboard() {
    ...
    const statCards = [
        ...
        { label: 'Total Admins', value: stats.totalAdmins, icon: '🛡️', color: '#7c3aed', bg: 'rgba(124,58,237,0.08)', sub: 'Managing content' },
        { label: 'Total Lessons', value: stats.totalLessons, icon: '📚', color: '#f59e0b', bg: 'rgba(245,158,11,0.08)', sub: 'In curriculum' },
        { label: 'Completions', value: stats.totalCompletions, icon: '✅', color: '#10b981', bg: 'rgba(16,185,129,0.08)', sub: 'Across all users' },
    ];
        ...
        <div>
            ...
            <div style={styles.pageHeader}>
                <div>
                    ...
                    <p style={styles.pageSubtitle}>Welcome back! Here's what's happening on TalkEnglish.</p>
                </div>
            ...
            <div className="responsive-actions-grid">
                {[
                    { label: 'Add User', icon: '➕👤', path: '/superadmin/users', color: '#3b82f6' },
                    { label: 'Add Admin', icon: '➕🛡️', path: '/superadmin/admins', color: '#7c3aed' },
                    { label: 'Create Lesson', icon: '➕📚', path: '/superadmin/lessons', color: '#f59e0b' },
                    { label: 'View Leaderboard', icon: '🏆', path: '/superadmin/leaderboard', color: '#10b981' },
                ].map((action, i) => (
                    <button
                        key={i}
            ...
            <div style={styles.sectionHeader}>
                ...
                <button style={styles.viewAllBtn} onClick={() => navigate('/superadmin/leaderboard')}>
                ...
                <table style={styles.table}>
                    <thead>
                        <tr>
                            ...
                            <th style={styles.th}>Lessons Done</th>
                    ...
                    <tbody>
                        {topScorers.length === 0 && (
                            <tr>
                                <td colSpan={6} style={styles.emptyCell}>No users yet.</td>
                            </tr>
                        )}
                        {topScorers.map((u, i) => (
                            <tr key={u.id} style={styles.tr}>
                                ...
                                <td style={styles.td}>
                                    <span style={{ ...styles.levelBadge, ...levelColors[u.level] }}>
                                        {levelColors[u.level]?.label || u.level}
                                    </span>
                                </td>
...
const styles = {
    ...
    viewAllBtn: { background: 'transparent', border: 'none', color: '#7c3aed', cursor: 'pointer', fontSize: 13, fontWeight: 600 },
    ...
    userAvatar: { width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: '#fff', flexShrink: 0 },
